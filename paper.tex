...
\title{A National Pipeline System as a Solution to Future Water Shortage in the United States}
...
analysis of how best to satisfy the growing water needs of the United States. 
...
of what kinds of infrastructure need to be developed, and how significant of a problem water shortages are likely to become in the forseeable future.
...
The more significant problem however, as was the case in the drought of 2012-2013, is that even when there is enough water available in the United States as a whole, it might be the case that while some regions are oversupplied, while others are severely in drought.
...
\item De-salinization plants, reservoirs, and purification plants are all facilities that are built at a single location, and have the intended effect of providing more water to that location. For the purposes of our modelling, we can think of them as being essentially the same structure, with the proviso that they each can only be constructed in particular parts of the country. 
...
per gallon per day\cite{key-12}. This figure is approximately the same as for similar kinds of facilities (reservoirs, dams).
...
All data analysis and visualization was performed using the technical computing language Julia \cite{julia}, making use of the packages "DataFrames", "Gadfly", "Distributions", "Compose", "Graphs" all of which are available in the Julia package manager and were licensed under the MIT license. 
...
As the problem of water distribution and availability is very much determined by the geographic location and conditions, we obtained a dataset containing the outlines and geographic locations of every county in the United States \cite{arcgis}, which we then used in our county level analysis. Figure \ref{fig:outline} shows a visualization of the contents of this data set. For the method of visualization used, see Section \ref{sec:vis}.

\subsubsection{Data Processing}
The dataset we obtained was in the proprietary, binary "Shapefile" format, that is used by many professional GIS systems. Luckily, there exists an open documentation of the format \cite{shapefile-spec}, which allowed us to implement our own parser to extract the locations, and shapes of the various counties. 
...
As outlined in Section 3, we made use various data points collected by the USGS (United States Geological Survey) for every single county in the United  States in the years 1985, 1990, 1995, 2000 and 2005. The data is split into 104 categories ranging from standard data points such as the total amount of water consumed in a specific specific county to more unexpected ones such as water irrigation due to golf courses. In addition to the actual data, every row also includes meta information such as the State in which the county is located and the FIPS (Federal Information Processing Standard) code of the county for which the data was collected.

The data is available from the USGS website in standard ASCII tsv format.

\subsubsection{Data Processing}
The standard format of the data allowed for an easy import into our computational software using the Julia "DataFrames" package. This allowed us to treat the data essentially as an im-memory database which we could query not only for the downloaded data, but which also allowed for more complex models to be executed on the data with relative ease. Treating the data as a database also tremendously helped with combining all the data from the many different sources. Not all of the data we obtained contained the same information as to their location, but after normalizing all the different kinds of information available to a FIPS code based primary key, standard database operations such as joining two datasets by primary key. This operation was incredibly helpful during visualization as we could just join the DataFrame containing the geographical information with that containing e.g. the water usage information and have the results be immediately 
...
\begin{figure}
       ...
       \begin{subfigure}[b]{0.4\textwidth}
                ...
                \label{fig:2000}
        \end{subfigure}
        \caption{USGS Data on total water consumption in the indicated year. Values are represent on a logarithmic scale with green indicating values towards the bottom of the scale and red indicating values towards the top of the scale.}\label{fig:water}
\end{figure}

In order to be able to visualize the input data and any simulation results obtained from the models used in this paper, we created a custom visualization software based on the Julia "Compose" package, a modern implementation of Wilkinson-Wickham's Grammar of Graphics. 

The visualization software can visualize arbitrary county level data sets as long as there is some way to identify  which county the data belongs to. This is easiest is the data in need of visualization is already keyed on the FIPS code of the county, but it is also possible to plot data that only contains longitudinal and latitudinal information of any point in the county, a feature which is very useful to visualize data coming from individual measurement stations within the county rather than values that are determined for the county as a whole. 

We extended our software to be able to graph arbitrary directed and undirected Graphs (represented by the Julia "Graphs") between counties our points within these counties. This capability was used to generate the graph of the proposed pipeline network.

Our software also makes use of the vector graphics capabilities of the Compose framework to create almost infinitely scalable graphics that allowed us to visualize smaller areas very effectively. The figures contained within this document are rasterizations of these vector images  to ensure a reasonable file size. 
...
Our data projections are subject to significant error on the local level. County data often contains isolated outliers and certain states are missing data in 2000 altogether. We used simple linear regression after removing for missing and anomalous data, but the presence of outliers and noise in such a small data set (four to five points) for each county and the distance of extrapolation magnify possible errors that could occur.

Second, our MST algorithm is based on several simplifying decisions that might introduce error into its calculations. For example, in practice each county has a central town or location that is not usually its geographical centroid, as we assumed for the purposes of the minimal spanning tree calculation. This would throw off our calculations of the spanning tree at the local level as well.

On the other hand, although these local errors may exist and even be large in relative magnitude, by summing them we have a robust large-scale model. Because standard deviation is subadditive the accumulated errors in summing to find the total demand should not be significant. Similarly, whereas local constructions of pipelines might be better optimizable than what we calculated, our calculated total length and cost should not be far off the actual value.

Finally, all expenses are estimated using numbers available online that are sensitive to local conditions, so there is room for error there as well. However, having checked multiple sources for each number these numbers should be within a reasonable constant factor of the true cost.
...
Our proposed pipeline network can play a huge role in managing the distribution of freshwater over the United States. However, it is not a perfect solution to water management problems, and has some potential flaws:
\begin{itemize}
\item Before completion, there are holes in the network: Before every pipeline has been constructed, 
there will be some pairs of counties that are not connected by pipelines. In this case, the network is still 
useful, but is not quite as powerful as a complete network would be.
\item Large scale droughts can be an issue: if the entirety of the one region (say, the west coast) is in 
drought, while (say) the east coast has a water surplus, there may be bottlenecks in the middle of the country 
as large amounts of water are transferred. This could potentially be an issue, but in reality, this kind of situation 
is extremely rare, partially because the coasts of the United States do not tend to be where droughts occur.
\item Some nearby counties are only connected by long and convoluted paths: This is not as much of an issue as 
one might imagine, because generally droughts happen slowly, and hence it is acceptable for water to take a long time to 
reach its destination. However, if this is seen as a major issue, a small number of additional pipes can be added on a case-by-case basis.
\end{itemize}

However, none of these is a fatal flaw in the idea of constructing a pipeline network.

\section{Avenues for Future Research}
A finer and more nuanced study of water usage and available data would provide a more complete model for water supplies and demands, as well as inform decisions about how to implement our proposals in reality. Here are some areas for future research:

\begin{itemize}
\item Groundwater level data is available in extreme detail from the US Geological Survey. Using this we can identify the areas of most severe exploitation as well as predict long-term aquifer lifespans. Areas with limited groundwater resources can then be prioritized in the building of the national pipelin network.
\item Research into currently available pipelines can be made if such data is collected; using these pipelines we predict that a large fraction of the network can simply be commandeered from existing infrastructure instead of built redundantly. This can be factored into the MST algorithm to give a minimal spanning tree based on pipelines already available. A problem with this approach is that available pipelines may not be of the condition or flow volume we require.
\item It is difficult with the granularity of the data available to actually model the optimal locations for specific de-salination plants and aqueducts to be built, although we can map out the general areas in which they are in need. The answer may perhaps be made on a case-by-case basis.
\item Reducing the total demand of the US is another approach that we did not address. New methods and technologies developed to this end should be used to inform our decisions about modeling the future demands of the US and decisions about pipeline-building.
\item Although we made the simplifying assumption that pipelines are the only largely available type of aqueduct, in reality in certain regions canals or other constructs may be more effective and less costly alternatives.
\end{itemize}
